...
Evaluation using Mean Reciprocal Rank (MRR) and Maximum Mean Discrepancy (MMD) shows that **CFP-GEN underperforms unconditional baseline** in terms of distributional alignment for long sequences (refer to Table <a href="#table-1">1</a>). Additionally, as the sequence length increases, both models experience a decline in performance (see Figure <a href="#figure-1">1</a>). These findings suggest a better solution to resolve long-context generation challenges in Language Models as a whole.
